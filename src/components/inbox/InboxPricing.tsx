...
const plans: Plan[] = [
  {
    ...
    bullets: [
      "3 posts semanais (imagens)",
      "2 reels mensais",
      "Captação realizada pela INBOX Agência",
      "Conteúdo com fotos fornecidas pela cliente",
    ],
  ...
  },
  {
    ...
    bullets: [
      "3 posts semanais (imagens)",
      "4 reels mensais",
      "Renovação de fotos e vídeos a cada 2 meses",
      "Gerenciamento estratégico do Instagram",
    ],
    ...
    <div className="grid gap-4 md:grid-cols-2">
      {plans.map((plan) => (
        <Card
          key={plan.title}
          className={
            "relative overflow-hidden border-border/70 bg-card/60 p-6 shadow-soft backdrop-blur " +
            (plan.highlight ? "ring-1 ring-primary/50" : "")
          }
        >
          {plan.highlight ? (
            <div className="absolute right-4 top-4 rounded-full border border-border/70 bg-background/20 px-3 py-1 text-xs text-primary">
              Recomendado
            </div>
          ) : null}
